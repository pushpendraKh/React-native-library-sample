...
internal func timedCoordinatesFromString(coordinateString: String) -> TimedCoordinates? {
    ...
    if components.count >= 3 {
        ...
        return TimedCoordinates(location: location, timeStamp: timeStamp, bearing: HTMapUtils.headingFrom(CLLocationCoordinate2D.zero, next: location))
    }
     return nil
   }
...
internal func timedCoordinatesFromStringArray(coordinatesString: String) -> [TimedCoordinates] {
    ...
    for coordinateString in stringArray {
        if let cordinate = timedCoordinatesFromString(coordinateString: coordinateString){
...
internal func timedCoordinatesFrom(polyline: String, precision: Double = 1e5) -> [TimedCoordinates]? {
    ...
    var lastCoordinate = CLLocationCoordinate2D.zero
    while position < length {
        ...
        let bearing = HTMapUtils.headingFrom(lastCoordinate, next: currentCoordinate)
        timedCoordinates.append(TimedCoordinates(location: currentCoordinate, timeStamp: date, bearing: bearing))
